...
3. 对于 sql/ciyacaSQL.sql ，在 MySQL 里创建一个名为 ciyacaSQL的数据库，并使用 source 命令将其导入
4. 在数据库导入完毕后，将 sql/sql.cpp中的 mysql_real_connect 函数连接参数填写正确（MySQL 登录账户、登录口令、端口、数据库名等）
...
1. 下载Qt 5.14.0
2. 安装时选择全部组件
3. 使用qt打开工程，选择client/serc/test.pro进行加载
4. 运行即可
...
1. 安装msgpack
  1. 请访问https://github.com/msgpack/msgpack-c/tree/cpp_master自行编译安装
2. 安装qt5.14.0
3. 安装mysql 5.7.30
4. 依据server说明配置数据库
...
.. list-table:: 消息备份历史记录
  ...
  * - .. figure:: ./_static/feature-history-1.png
        :figwidth: 300px
    - 在13：01分 `用户q` 向 `用户w` 发送了四条消息，此时 `用户w` 没有登录上线。

  * - .. figure:: ./_static/feature-history-2.png
        :figwidth: 300px
    - `用户w` 在 13:21 登录。 

  * - .. figure:: ./_static/feature-history-3.png
        :figwidth: 300px
    - `用户w` 能够点击消息记录按钮查看历史消息。
...
- 服务端逻辑代码模块，提供服务端应用逻辑。详见 `服务端设计`_ 。
